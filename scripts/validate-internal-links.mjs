...
// Each live calculator is rendered inside exactly one orienting cluster hub.
// This is stronger than a flat directory link because the hub explains when
// to use the tool and what to compare next.
for (const tool of liveTools) addInbound(`/tools/${tool.slug}`, '/tool-hubs/[slug]', 'cluster-hub');
