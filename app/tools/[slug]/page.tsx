  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      ...
      <header className="mt-8 grid gap-6 lg:grid-cols-[1fr_0.5fr] lg:items-end">
        <div>
          <span className="rounded-full bg-sky-100 px-4 py-2 text-xs font-bold uppercase tracking-wide text-sky-700">{tool.category}</span>
          <h1 className="mt-5 max-w-4xl text-4xl font-black tracking-tight text-slate-950 md:text-5xl">{tool.name}</h1>
          <p className="mt-5 max-w-3xl text-lg leading-8 text-slate-600">{tool.shortDescription}</p>
        </div>
        <div className="rounded-3xl border border-amber-200 bg-amber-50 p-5 text-sm leading-6 text-amber-900">
          <p className="font-bold">Educational estimate only</p>
          <p className="mt-2">Calculator results can vary based on company policy, lender terms, tax law, investment assumptions or personal details.</p>
        </div>
      </header>
        ...
        <aside className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
          ...
          <div className="mt-5 grid gap-3">
            {related.length > 0 ? related.map((item) => (
              <Link key={item.slug} href={`/tools/${item.slug}`} className="rounded-2xl border border-slate-200 p-4 transition hover:border-sky-200 hover:bg-sky-50">
